...
public class ZhiCoreExceptionFilter : AbpExceptionFilter
{
    ...
    private WrapResult<object> SimplifyMessage(ExceptionContext context)
    {
        ...
        var localizer = context.GetRequiredService<IStringLocalizer<ZhiCoreLocalizationResource>>();
        switch (context.Exception)
        {
            case AbpAuthorizationException:
                result.SetFail(localizer["My.ZhiCore:PermissionDenied"], "401");
                break;
            case AbpValidationException validationException:
                var errorMessage = localizer["My.ZhiCore:ParameterValidationFailed"] + ";" + validationException.ValidationErrors.JoinAsString(";");
                result.SetFail(errorMessage, "400");
                break;
            case EntityNotFoundException:
                result.SetFail(localizer["My.ZhiCore:EntityNotFound"], "506");
                break;
            case NotImplementedException:
                result.SetFail(localizer["My.ZhiCore:Unimplemented"], "507");
                break;
            case DbUpdateConcurrencyException:
                result.SetFail(localizer["My.ZhiCore:DbUpdateConcurrency"], "508");
                break;
            default:
            {
                if (context.Exception is IHasErrorCode codeException)
                {
                    var exceptionConverter = context.GetRequiredService<IZhiCoreExceptionConverter>();
