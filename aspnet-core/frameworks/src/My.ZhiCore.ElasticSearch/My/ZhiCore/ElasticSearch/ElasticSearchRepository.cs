    ...
    where TEntity : class, IElasticSearchEntity
{
    ...
    /// 根据id查询实体
    /// </summary>
    /// <exception cref="ZhiCoreElasticSearchEntityNotFoundException"></exception>
    ...
    public virtual async Task<TEntity> GetAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        if (!result.IsValid)
        {
            throw new ZhiCoreElasticSearchEntityNotFoundException(innerException: result.OriginalException);
        }
    ...
    /// </summary>
    /// <exception cref="ZhiCoreElasticSearchException"></exception>
    public virtual async Task InsertAsync(TEntity entity, CancellationToken cancellationToken = default)
    {
        var result = await Client.IndexAsync(entity, x => x.Index(IndexName), cancellationToken);
        if (!result.IsValid)
        {
            throw new ZhiCoreElasticSearchException(innerException: result.OriginalException);
        }
    }
